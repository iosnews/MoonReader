...
var newname='';
qq.getthesize = function(){
   var time = new Date().getTime();
   var url = '/getFileSize/'+newname;
	$.getJSON(url, function(data) {
		getProgressReties ++
		if(!data)
		{
			document.getElementById("fileSize").innerHTML=data.size;
		}
		if (getProgressReties < 5) {
		alert(getProgressReties);
			setTimeout(qq.getthesize,500);
		}
});			
};
...
qq.FileUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        template: '<div class="qq-uploader">' +
                ...
                '<div class="qq-upload-button"><img src="/root/btn_ch.gif"/></div>' +
		...
		fileTemplate: '<div style = "background: url(\'/root/bg1.jpg\') repeat-x; height: 40px; position: relative; color: #fff;">' +
						...
						'<strong class = "fileSize"  id="fileSize" style = "display: inline-block; position: absolute; left: 433px; width: 50px; height: 40px; line-height: 40px; z-index: 3; font-style: normal;"></strong>'+
						'<span class="qq-upload-spinner" style = "display: none; position: absolute; left: 50%; width: 105px; height: 16px; line-height: 40px; z-index: 3; font-style: normal; top: 12px;"></span>' +
						...
						'<a class="qq-upload-cancel" href="javascript:;" style ="position: absolute; z-index: 2; right: 20px; height: 40px; line-height: 40px;">取消</a>'+
						'<span class="qq-upload-failed-text qq-upload-fail" style ="position: absolute; z-index: 2; right: 0; height: 40px; line-height: 40px;">上传失败</span>'+
...
qq.extend(qq.UploadHandlerForm.prototype, {
	...
	$('.qq-upload-spinner').show();
		...
		var time = new Date().getTime();
        	newname=fileName; 
       // setTimeout(function(){
       document.getElementById("fileSize").innerHTML="上传中...";
        form.appendChild(input);
        //  }, 100);
