...
+(NSMutableDictionary *)sd_anmaiteGIGArrWithdata:(NSData *)data{
    if (!data)
    {
        return nil;
    }
...
//        
    ...
    CFRelease(source);
    [dic setObject:images forKey:@"arr"];
    [dic setObject:[NSString stringWithFormat:@"%.1f",duration] forKey:@"time"];
...
+ (NSMutableDictionary *)sd_animatedGIFArrNamed:(NSString *)name
{
    ...
    if (scale > 1.0f)
    {
        ...
        if (data)
        {
            return [UIImage sd_anmaiteGIGArrWithdata:data];
        }
    }
    return nil;
}
